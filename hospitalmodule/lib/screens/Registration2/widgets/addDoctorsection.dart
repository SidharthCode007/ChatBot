...
import 'package:hospitalmodule/Models/regscreen2/addDoctorModel.dart';
import 'package:hospitalmodule/Models/regscreen2/specialisationModel.dart';
            ...
            ? Column(
                children: [
                  ...
                  children: [
                    ...
                    if (index < list.length - 1) // Add space for all items except the last one
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(widget.title, style: TextStyle(fontSize: 14,fontWeight: FontWeight.w700, color:border2 ),),
                IconButton(
                  onPressed: () {
                    setState(() {
                  ...
                  icon: Icon(Icons.add, color: border2,),
                )
              ],
            ),
...
class DocSubSection extends StatefulWidget {
  ...
  DocSubSection({Key? key, required this.doctorDetails,})
      : super(key: key);
...
class _DocSubSectionState extends State<DocSubSection> {
  ...
  DoctorDetailsSubmit d1 = DoctorDetailsSubmit(title: '', doctorName: '', qualification: '', selectedDays: []);


  @override
  Widget build(BuildContext context) {
    return     
      doctorAdded? added(submitteddetails: d1,):
      ...
      child: Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            controller: doctornameController //widget.doctorControllers[widget.doctorDetails.qualification],
          ),
          h5,
          Text("Add available day", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500,color:border2 ),),
          h5,
          buildDayDropdown(),
          h10,
          Center(
            child: ElevatedButton(onPressed: () {
              widget.doctorDetails.doctorName = widget.doctorDetails.doctorName;
              doctorAdded = false;
              submit(widget.doctorDetails);
            }, child: Text("Add")),
          ),
        Divider(thickness: 5,color: border2,)
        ],
      ),
    );
  }
  ...
  void submit(DoctorDetails details){
     d1 = DoctorDetailsSubmit(title: details.title, doctorName: doctornameController.text, qualification: qualificationController.text, selectedDays: widget.doctorDetails.selectedDays);
